...
class PickerDialog: UIView, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    typealias PickerCallback = (_ value: (String, String)) -> Void
    ...
    private var selectedPickerValue: (String,String)?
...
//    }
    ...
    func findIndexForValue(value: (String, String), array: [[String]]) -> (Int, Int)? {
        var first = -1
        var second = -2
        for (index, val) in array[0].enumerated() where val == value.0 {
            first = index
        }
//        for (index, val) in array[1].enumerated() where val == value.1 {
//            second = index
//        }
        return (first, second)
    }
    
    func findIndexForSearchText(value: (String, String), array: [[String]]) -> (Int, Int)? {
        var first = -1
        var second = -2
        for (index, val) in array[0].enumerated() where val.contains(value.0) {
            first = index
        }
//        for (index, val) in array[1].enumerated() where val == value.1 {
//            second = index
//        }
    ...
    func show(isShowSearchTextField: Bool = false, title: String, doneButtonTitle: String = "Выбрать", cancelButtonTitle: String = "Отмена", options: [[String]], selected: (String, String)? = nil, callback: @escaping PickerCallback) {
        ...
        if selected != nil {
            ...
            let selectedIndex = findIndexForValue(value: selected!, array: options) ?? (0,0)
            self.picker.selectRow(selectedIndex.0, inComponent: 0, animated: false)
//            self.picker.selectRow(selectedIndex.1, inComponent: 1, animated: false)
        }
        
        /* Show SearchTextField and hide title*/
...
//        NotificationCenter.default.addObserver(self, selector: Selector(("deviceOrientationDidChange:")), name: UIDevice.orientationDidChangeNotification, object: nil)
    ...
    private func createContainerView() -> UIView {
        ...
        let dialogContainer = UIView(frame: CGRect(x: (screenSize.width - dialogSize.width) / 2, y: (screenSize.height - dialogSize.height) / 3, width: dialogSize.width, height: dialogSize.height))
        ...
        //SearchTextField
        self.searchTextField = UISearchTextField(frame: CGRect(x: 10, y: 10, width: 280, height: 40))
        self.searchTextField.isHidden = true
        dialogContainer.addSubview(searchTextField)
...
//        self.titleLabel.isHidden = false
        ...
        self.picker.setValue(UIColor.black, forKeyPath: "textColor")
    ...
    private func addButtonsToView(container: UIView) {
        ...
        self.cancelButton.frame = CGRect(
            ...
            self.cancelButton.setTitleColor(UIColor.black, for: UIControl.State.normal)
            self.cancelButton.setTitleColor(UIColor.black, for: UIControl.State.highlighted)
        ...
        self.cancelButton.addTarget(self, action: #selector(doneButtonTapped), for: UIControl.Event.touchUpInside)
        ...
        self.doneButton.tag = kPickerDialogDoneButtonTag
            self.doneButton.setTitleColor(UIColor.black, for: UIControl.State.normal)
            self.doneButton.setTitleColor(UIColor.black, for: UIControl.State.highlighted)
        ...
        self.doneButton.addTarget(self, action: #selector(doneButtonTapped), for: UIControl.Event.touchUpInside)
        container.addSubview(self.doneButton)
    }

    @objc func doneButtonTapped(sender: UIButton!) {
        if sender.tag == kPickerDialogDoneButtonTag {
            let selectedIndex1 = self.picker.selectedRow(inComponent: 0)
            let selectedValue1 = self.pickerData[0][selectedIndex1]
//            let selectedIndex2 = self.picker.selectedRow(inComponent: 1)
//            let selectedValue2 = self.pickerData[1][selectedIndex2]
            self.callback?((selectedValue1, ""))
        }
...
extension PickerDialog: UISearchTextFieldDelegate {
    func textFieldShouldReturn(_ textField: UITextField) -> Bool {
        guard let text = searchTextField.text else { return true }
        searchTextField.resignFirstResponder()
        print("ShouldReturn = \(text)")
        return true
    }

    func textFieldDidChangeSelection(_ textField: UITextField) {
        guard let text = searchTextField.text else { return }
        let selectedIndex = findIndexForSearchText(value: (text, ""), array: pickerData) ?? (0,0)
        self.picker.selectRow(selectedIndex.0, inComponent: 0, animated: true)
        print("DidChangeSelection = \(text)")
    }
}
